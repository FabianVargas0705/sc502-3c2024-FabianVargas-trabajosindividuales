document.addEventListener('DOMContentLoaded', function () {
    ...
    const APICOM_URL = "backend/comments.php";
    ...
    async function loadAllComments() {
        for (let task of tasks) {
            try {
                const response = await fetch(`${APICOM_URL}?task_id=${task.id}`, {
                    method: 'GET',
                    credentials: 'include'
                });
                if (response.ok) {
                    ...
                    task.comments = [];
                    console.error(`Error al cargar comentarios para tarea ${task.id}`);
                }
            } catch (err) {
                task.comments = [];
                console.error(`Error al cargar comentarios para tarea ${task.id}:`, err);
            }
        }
    }
    ...
    function renderTasks() {
        ...
        tasks.forEach(function (task) {
            ...
            if (task.comments && task.comments.length > 0) {
                ...
                task.comments.forEach(comment => {
                    ...
                    <button type="button" class="btn btn-sm btn-link remove-comment" data-taskid="${task.id}" data-commentid="${comment.id}">Delete</button>
                    </li>`;
                });
                commentsList += '</ul>';
            }
        ...
        document.querySelectorAll('.remove-comment').forEach(function (button) {
            button.addEventListener('click', async function (e) {
                ...
                try {
                    const response = await fetch(`${APICOM_URL}?id=${commentId}`, {
                        method: 'DELETE',
                        credentials: 'include'
                    });
    ...
    document.getElementById('comment-form').addEventListener('submit', async function (e) {
        ...
        try {
            const response = await fetch(APICOM_URL, {
                method: 'POST',
                credentials: 'include',
                headers: {
